...
todo_list = [
 ...
 ] # hint(리스트가 아니라 중괄호로 싸져 있었음)

# todos_list 컬렉션 생성 후 collection_todos 변수에 담기
collection_todo = todolist_functions.Connect_Mongo("todo_list")          # hint(todos_list -> todo_list)
...
todolist_functions.Data_insert(collection_todo, todo_list)         # hint(collection_todos -> collection_todo),function(data_insert로 변경)

# 종료 여부 입력 function 호출
# user_end = 'q' --> User_name function 호출 --> Todos function 호출 --> 종료 여부 입력 (반복) (collection_todos -> collection_todo)
